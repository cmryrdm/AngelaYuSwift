...
class ChatViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    customButton.setImage(UIImage(systemName: "rectangle.portrait.and.arrow.right"), for: .normal)
    customButton.addTarget(self, action: #selector(self.logout), for: .touchUpInside)
    ...
    navigationController?.navigationBar.barTintColor = UIColor(named: Const.BrandColors.blue)
  ...
  func loadMessages() {
    ...
    // instead of getDocuments, addSnapshotListener is used for real time update.
          ...
          if let snapshotDocuments = querySnapshot?.documents {
            for doc in snapshotDocuments {
              ...
              if let messageSender = data[Const.FStore.senderField] as? String, let messageBody = data[Const.FStore.bodyField] as? String {
                ...
                // hold on to main asynchronously to reload tableview contents !!!
  ...
  @IBAction func sendPressed(_ sender: UIButton) {
    if let messageBody = messageTextfield.text, let messageSender = Auth.auth().currentUser?.email {
      db.collection(Const.FStore.collectionName).addDocument(data: [
        ...
        if let err = error {
          print("There was an issue savin data to Firestore, \(err)")
        } else {
...
extension ChatViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    cell.label.text = messageRow.body
    
    // This is a message from current user
    if messageRow.sender == Auth.auth().currentUser?.email {
